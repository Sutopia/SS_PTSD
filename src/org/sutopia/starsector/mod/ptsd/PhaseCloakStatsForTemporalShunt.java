...
public class PhaseCloakStatsForTemporalShunt extends PhaseCloakStats {
	...
	public static float getMaxTimeMult(MutableShipStatsAPI stats) {
		if (stats.getVariant().hasHullMod(PTSD_HULLMOD_ID)) return 1f;
	...
	public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
		...
		boolean hasPTSD = stats.getVariant().hasHullMod(PTSD_HULLMOD_ID);
		...
		if (hasPTSD && (state == State.ACTIVE || state == State.OUT || state == State.IN)) {
			...
			//stats.getAcceleration().modifyPercent(id + "_2", MANEUVERABILITY_BONUS);
			//stats.getDeceleration().modifyPercent(id + "_2", MANEUVERABILITY_BONUS);
			//stats.getTurnAcceleration().modifyPercent(id + "_2", MANEUVERABILITY_BONUS * 2f);
			//stats.getMaxTurnRate().modifyPercent(id + "_2", MANEUVERABILITY_BONUS);
		}
	...
	protected float getDisruptionLevel(ShipAPI ship) {
		if (FLUX_LEVEL_AFFECTS_SPEED) {
			...
			float hardFluxLevelForPenalty = Math.max(0f, 
					ship.getHardFluxLevel() - (ship.getVariant().hasHullMod(PTSD_HULLMOD_ID) ? PTSD_NO_PENALTY_FLUX_LEVEL : 0f));
			
			float level = hardFluxLevelForPenalty / threshold;
	...
	protected void maintainStatus(ShipAPI playerShip, State state, float effectLevel) {
		...
		if (cloak == null) return;
		boolean hasPTSD = playerShip.getVariant().hasHullMod(PTSD_HULLMOD_ID);
			...
			if (level > f) {
				if (getDisruptionLevel(playerShip) <= 0f) {
					Global.getCombatEngine().maintainStatusForPlayerShip(STATUSKEY3,
							...
							hasPTSD ? "zero flux bonus at 100%" : "top speed at 100%", false);
				} else {
					String speedPercentStr = (int) Math.round((hasPTSD ? 
							getZeroFluxSpeedMult(playerShip, effectLevel) : 
							getSpeedMult(playerShip, effectLevel)) * 100f) + "%";
					Global.getCombatEngine().maintainStatusForPlayerShip(STATUSKEY3,
							cloak.getSpecAPI().getIconSpriteName(),
							"phase coil stress", 
							(hasPTSD ? "zero flux bonus at " : "top speed at ") + speedPercentStr, true);
				}
			}
		}
	}
